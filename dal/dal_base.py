...
根据k=v 判断属性  缓存处理
'''
from dbs.mysqlapp import MySQLApp  ## 数据库的连接通道
...
class Dal_base:
        ...
        for k, v in kwargs.iteritems():
            if md.__mappings__[k].innertype == 'int':
                ...
                if isinstance(v,unicode): ##判断是否已经编码，如果没有就utf-8编码
                    v = v.encode('utf-8')
                v = str(v)
            elif md.__mapping__[k].innertype == 'float':
                v = float(v)
    ...
    # def getPageData(self,table,pageid):
    # if (pageid in self._m_cache) == False:
    #     pagenum = 3
    #     pageid = int(pageid)
    #     start = (pageid - 1) * pagenum
    #     Instance = MySQLApp().getInstance()
    #     # sql = 'select * from %s order by %s desc limit %d,%d'%(table,id,start, pagenum)
    #     sql = 'select * from %s limit %d,%d' % (table, start, pagenum)
    #     res = Instance.query(sql)
    #     if res == None:
    #         return None
    #     # 如果在数据库中 就把它加入到缓存的当中
    #     self._m_cache[pageid]=res
    # return self._m_cache[pageid]
